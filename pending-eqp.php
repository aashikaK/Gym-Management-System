<?php
    ...
    <input type="hidden" name="req_uid" value="<?= htmlspecialchars($req_userid); ?>">
            ...
            if (isset($_POST['approve_eqp'])) {
                ...
                $updatePendingEqp = "UPDATE pending_equipment SET status='complete', approved_date=NOW() WHERE id='$reqid'";
                ...
                $paySql="UPDATE eqp_payment
                SET status = 'complete'
                WHERE user_id=$userid AND e_id='$eid'";
                mysqli_query($conn,$paySql);

                $message = "Your request for buying $EquipmentName has been approved. The equipment will be delivered soon"; 
            ...
            if (isset($_POST['reject_eqp'])) {
                ...
                $message = "Your request for buying $EquipmentName was denied."; 
                $insertNotification = "INSERT INTO notifications (user_id, message, section) VALUES ('$userid', '$message','prod_equip')";
                mysqli_query($conn, $insertNotification);
